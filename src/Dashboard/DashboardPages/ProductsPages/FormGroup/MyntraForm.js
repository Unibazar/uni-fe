...
function MyntraForm({ handleSubmit, control, ProductPhotoSrc1, productPhoto1, ProductPhotoSrc2, productPhoto2, ProductPhotoSrc3, productPhoto3, ProductPhotoSrc4, productPhoto4, errors, watch, setValue }) {
  ...
  const onSubmit = data => {
    ...
    const payload = {
      txnId: `TXN-${Date.now()}`,
      reqId: `REQ-${Date.now()}`,
      sellerId: userData?.myntra?.sellerId || "db8de917d2734d39", // This should be dynamically set based on the logged-in seller
      skuId: data.skuId || "ABCDEE2322",
      quantity: 10,
      processingSla: 5,
      store_code:"MF",
      vertical: "book",
